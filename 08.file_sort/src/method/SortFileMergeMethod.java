...
public class SortFileMergeMethod {
    ...
    private static int[] sortAndMergeTwoChunks(String chunkSource, int posChunk1, int posChunk2) throws IOException {
        int[] chunk1 = readAndMergeSortChunk(chunkSource, posChunk1);
        int[] chunk2 = readAndMergeSortChunk(chunkSource, posChunk2);
    ...
    private static int[] readAndMergeSortChunk(String chunkSource, int pos) throws IOException {
        int[] sortedChunk = File.readNumbers(chunkSource, pos, CHUNK_SIZE);
        MergeSort.sort(sortedChunk, 0, CHUNK_SIZE - 1);
        return sortedChunk;
    }
}
